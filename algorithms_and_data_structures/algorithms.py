...
def isprime_diap(n: int) -> list[int]:
    primes = [True] * (n + 1)
...
def factor(n: int) -> list[int] | None:
    result = []
    d = 2
...
def quick_sort(nums: list[int]) -> list[int]:
    ...
    return quick_sort(left) + [e for e in nums if e == pivot] + quick_sort(right)
...
# Рекурсия
# По сути это функция, которое в процессе выполнение выполняет себя столько раз, пока не будет собдлюдено условие
...
if __name__ == "__main__":
    ...
    # print(heap.heap_sort([8, 5, 3, 7, 7, 2]))
    # print(heap_sort([8, 5, 3, 7, 7, 2]))
    # print(find_num_between_target(
    #     [1, 2, 3, 1234, "asdfas", 12341234, 4, 5, 2], 2))
    #
