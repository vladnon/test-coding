...
class WindowGame():
    ...
    def create_other_things(self):
        self.user = Game()
        self.balance = CTkLabel(self.window, font=CTkFont(
            family='Benzin-Bold', size=15), text=f'Баланс: {self.user.coins}', text_color='white')
        self.user_sign = CTkLabel(self.window, image=self.paper_img, text=None)
        self.enemy_sign = CTkLabel(
            self.window, image=self.paper_img, text=None)
        self.res = CTkLabel(self.window, text='Нет',  font=CTkFont(family='Benzin-Bold', size=15),
                            anchor=CENTER, fg_color="transparent", bg_color="transparent", text_color='white')
        self.bet_entry = CTkEntry(self.window, placeholder_text='Ставка', font=CTkFont(
            family='Benzin-Bold', size=10), text_color='white', width=80, fg_color="transparent", bg_color="transparent")
        self.max = CTkLabel(self.window, text=10, font=CTkFont(
            family='Benzin-Bold', size=15), fg_color="transparent", bg_color="transparent", text_color='white')

    def create_btns(self):
        self.paper_b = CTkButton(self.window, image=self.paper_img,  command=self.paper,
                                 text=None, fg_color="transparent", hover=False, bg_color="transparent")
        self.stone_b = CTkButton(self.window, text=None,  command=self.stone,
                                 fg_color='transparent', image=self.stone_img, hover=False, bg_color="transparent")
        self.scissors_b = CTkButton(self.window, text=None,  command=self.scissors,
                                    image=self.scissors_img, fg_color='transparent', hover=False, bg_color="transparent")

    def create_img(self):
        self.paper_img = CTkImage(light_image=Image.open(
            './something like game/img/paper.png'), size=(125, 91))
        self.stone_img = CTkImage(light_image=Image.open(
            './something like game/img/stone.png'), size=(130, 91))
        self.scissors_img = CTkImage(light_image=Image.open(
            './something like game/img//scissors.png'), size=(87, 91))
